...
void chi2(int &npar, double *gin, double &f, double *par, int iflag){
  ...
  for(int i = 0; i < data::x1.size(); i++){
	  ...
	  //abbiamo 6 parametri, dobbiamo dire quali devono essere usati per f1 e quali per f2
  }
}
...
int main(){
  ...
  // Minuit
  TMinuit *m = new TMinuit(6);
  m->SetFCN(chi2);
  m->DefineParameter(0, "x0",36,0.01,0.,0.);
  m->DefineParameter(1, "T0",4.0,0.01,0.,0.);
  m->DefineParameter(2, "a1",f1->GetParameter(2),0.01,0.,0.);
  m->DefineParameter(3, "b1",f1->GetParameter(3),0.01,0.,0.);
  m->DefineParameter(4, "a2",f2->GetParameter(2),0.01,0.,0.);
  m->DefineParameter(5, "b2",f2->GetParameter(3),0.01,0.,0.);
  m->Command("MIGRAD");
